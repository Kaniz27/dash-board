            ...
            <div key={post.id} className="bg-white rounded-lg shadow overflow-hidden relative">
              ...
              <div className="flex border-t border-gray-200 relative">
                <button
                  ...
                  onClick={() => setSelectedLikes(selectedLikes === post.id ? null : post.id)}
                >
                  👍 <span>{post.likes.length}</span>
                </button>
                <button
                  className="flex-1 flex items-center justify-center gap-1 p-3 hover:text-[#01cdcc]"
                  onClick={() => setSelectedComments(selectedComments === post.id ? null : post.id)}
                >
                  💬 <span>{post.comments.length}</span>
                </button>
                <button className="flex-1 flex items-center justify-center gap-1 p-3 hover:text-[#01cdcc]">
                  ↗️ <span>{post.shares}</span>
                </button>
              </div>
